...
### 🥇 但更好的做法：不要填單色，用 clone 平鋪

alpha-aware 只解決「圓角被塗方」，**解決不了色差接縫**——元件底色通常帶細微紋理與上下明暗變化，
填單色會在塗抹邊界留下一條**可見直線**（踩過：指揮盒右側那條線）。

**正解：取一條乾淨的垂直窄帶，水平平鋪。** 保留 y 方向的所有明暗變化，只消除 x 方向的文字：

```python
# 1) 先找乾淨窄帶：水平變異最小的 x 區間
for x0 in range(...):
    strip = arr[y1:y2, x0:x0+8, :3]
    var = np.abs(strip - strip.mean(axis=1, keepdims=True)).mean()
    # var < 3 ＝ 這條沒有文字，可當來源

# 2) 先複製來源（避免被自己覆蓋），再水平平鋪
strip = arr[y1:y2, 294:302, :3].copy()
for x in range(x_start, x_end, 8):
    w = min(8, x_end - x)
    arr[y1:y2, x:x+w, :3] = strip[:, :w]     # 只改 RGB，alpha 不動
```

**驗證方式**：塗完沿一條橫線掃描列印顏色，相鄰取樣點的 Δ 應該全是 0；哪裡有跳變，哪裡就是接縫。

### ⚠️ 塗抹前先量到要保留的元素邊界

踩過：以為圖示只到 x=86 就從那裡開始塗，**把圖示右側的一部分削平了**（城堡的小塔與樹叢），
畫面上看起來只是「圖比較窄」，不會馬上發現。

- 先把原圖用**棋盤格底 ＋ 放大 3 倍**輸出來看，確認每個元素真正的邊界
- 塗完用差異驗證，確認只動到該動的地方：

```python
d = np.abs(orig - now).sum(axis=2)
ys, xs = np.where(d > 20)
print(f"實際改動範圍 x={xs.min()}-{xs.max()} y={ys.min()}-{ys.max()}")
print("要保留區的差異總和:", int(d[:, :x_start].sum()), "← 必須是 0")
```
